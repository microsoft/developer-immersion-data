...
namespace CreateSampleData
{
    public class SearchServiceDataInitializer
    {
        ...
        {
            await Initialize(_indexer, "indexend.json", true);
        }

        public async Task Initialize(string indexer, string indexfile, bool createSynonym)
        {
            _indexer = indexer;
        ...
        async Task CreateIndex(string indexfile)
        {
            ...
            using (var _httpClient = new HttpClient())
            {
                ...
                string file = File.ReadAllText($"sampledata/searchservice/{indexfile}");
