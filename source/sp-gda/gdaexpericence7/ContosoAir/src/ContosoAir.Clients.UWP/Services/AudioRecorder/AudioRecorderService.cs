...
using ContosoAir.Clients.UWP.Services.AudioRecorder;
using System;
...
namespace ContosoAir.Clients.UWP.Services.AudioRecorder
{
    class AudioRecorderService : IAudioRecorderService
    {
        ...
        async Task StartRecordingAsync()
        {
            try
            {
                recordingFile = await ApplicationData.Current.LocalFolder.CreateFileAsync(GlobalSettings.AudioFilename, CreationCollisionOption.ReplaceExisting);
