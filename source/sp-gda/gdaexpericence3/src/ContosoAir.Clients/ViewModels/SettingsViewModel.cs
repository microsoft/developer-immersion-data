...
namespace ContosoAir.Clients.ViewModels
{
    public class SettingsViewModel : ViewModelBase
    {
        ...
        public string FacebookBotAccountId
        {
            get { return Settings.FacebookBotAccountId; }
            set
            {
                Settings.FacebookBotAccountId = value;
                RaisePropertyChanged(() => FacebookBotAccountId);
            }
        }
        ...
        // Set Properties to Read GloabalSettings

        public string GetSoloServiceInfoUrl
        {
            get { return Settings.GetSoloServiceInfoUrl; }
            set
            {
                if (!value.Equals(Settings.GetSoloServiceInfoUrl))
                {
                    Settings.GetSoloServiceInfoUrl = value;
                    RaisePropertyChanged(() => GetSoloServiceInfoUrl);
                }
            }
        }
        ...
        {
            get { return Settings.GetSoloServiceReviewUrl; }
            set
            {
                if (!value.Equals(Settings.GetSoloServiceReviewUrl))
                {
                    Settings.GetSoloServiceReviewUrl = value;
                    RaisePropertyChanged(() => GetSoloServiceReviewUrl);
                }
            }
        }
