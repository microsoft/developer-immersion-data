    ...
    request(soloServiceDataUrl, function (error, response, body) {
        if(error) {
            ...
            var solo_service_providers = payload_start +
                                         '<text id="1">Solo Service Providers</text>'+
            ...
            var pick_from_airport = payload_start +
                                    '<text id="1">Your kid has been picked up by '+ soloService_1 +'</text>' +
                                    payload_end;

            // Notification Payload 3                         
            var drop_in_flight =  payload_start +
                                  '<text id="1">Your kid is boarded successfully !! was dropped by '+ soloService_1 +'</text>' +
                                  payload_end;
            
            // Notification Payload 4
            var pick_from_flight = payload_start +
                                  '<text id="1">Your kid has been picked up by '+ soloService_2 +' from flight</text>' +
                                   payload_end;

            // Notification Payload 5
            var drop_to_parent =  payload_start +
                                 '<text id="1">Your kid has been dropped successfully to his/her guardian by '+ soloService_2 +'</text>' +
                                  payload_end;
