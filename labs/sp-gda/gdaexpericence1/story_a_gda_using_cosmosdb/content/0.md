...
### Part B - Deployment of Resources Through Ansible Playbooks
   ...
   > _Here first of all we will see how we can deploy resources such as Azure Cosmos DB Account and Azure functions through Ansible Playbooks._

1. Launch **Bash on Ubuntu on Windows**. Click on **Ubuntu bash Icon** ![](img/ubuntuIcon.png). Bash shell will be launched. 
1. Follow the given steps to install **Ubuntu on Windows**. Press **y** to continue. This process may take some time.
![Fbash](img/fbash.png)
1. After completion of process close the **Ubuntu bash** and relaunch **Bash on Ubuntu on Windows** by clicking on **Ubuntu bash Icon** ![](img/ubuntuIcon.png). And then continue with the given steps.
...
- hosts: localhost
  vars:
      ad_user: testuser0001@click2cloud.onmicrosoft.com
      password: "Pogo#56789"
      subscription_id: b4cdcdb5-cd13-4b0b-9765-bb0eeef44b59
  tasks:
...
1. Add **Username** and **Password** given below against respective keys in the same **.yml** file.
   ...
   ```
1. **ad_user:** <inject key="AzureAdUserEmail" /> 
...
1. Press **Esc key + : + wq** to save the file in vi editor.
...
1. Once the playbook file is saved execute the given command to run the **Ansible playbook**.
   ```cmd
    ...
    ```    
1. Now go to **Azure Portal** click on **Resource groups** option present in the favourites panel. Resource group with all the resources which were deployed through **Ansible playbook** will appear.
   > Great! Just now you learnt how to deploy resources through Ansible playbooks.
...
1. To download the ContosoAir app source code launch the **Command Prompt** and run below commands
   ...
   ```
1. Now goto **c:\source\experience1** folder through **File Explorer** present on the task bar ![Solution Explorer](img/SolutionExplorer.png) and double click on **ContosoAir.Website_and_Services.sln** file to load the project into **Visual Studio 2017** IDE.  This may take some time.
...
1. Navigate to **Azure Portal's** **Resource groups** option present in the favourites menu on the left side panel and select the resource group ** ** and click on **Azure Cosmos DB Account** **<inject story-id="story://Content-Private/content/dfd/SP-GDA/gdaexpericence1/story_a_gda_using_cosmosdb" key="cosmosDBWithSQLDBName"/>** then, click on **Replicate data globally** option present under **** blade.
...
1. Go to **Solution Explorer** window, right click on "**ContosoAir.Services"** project and click on **Open folder in file explorer** option and copy the path of the folder.

   ![](img/OpenFolderInFileExplorer.jpg)

1. Open **Command prompt** window ![](img/CommandPrompt.jpg)  and type command: cd (Copied path from **Step 9**)

   **For e.g.** >cd Copied path from **Step 9** 
...
1. Enter **Document parameter name** as **"SelectDealsData"** in text box, **Database name** as **"contosoairdb"** and Collection name as **"DealsCollection"** in respective textboxes.
...
1. On clicking **</> Get function URL,** you will get a pop up window with a URL.
    ...
    ![](img/url.jpg)
1. Click on ![](img/copyIcon.jpg) icon to copy the given URL and paste it against **config variable** AZURE\_FUNCTION\_DEALS\_URL: 'Get function URL', in **config.js file** opened in **Visual Studio 2017** IDE and save the file.
...
1. Go to **Command Prompt** which was already launched in **Step 8 of Scenario 1, Part B.** and type command **npm start** to start the service.
   ```cmd
   npm start
   ```
...
2. You may encounter a popup for **Windows Firewall ...**. Click on cance button to close the pop up.
1. Go to **Solution Explorer** window, right click on "**ContosoAir.Website"** project and click on **Open folder in file explorer** option and copy the path of the folder.

   ![](img/explorer.jpg)
1. Open **Command prompt** window ![](img/CommandPrompt.jpg)  and type command: cd <Copied path from **Step 2**>

1. Once the above process is completed. Then go to **second Command prompt terminal** and get redirected to the copied path.
    
   **For eg:**   cd  Copied path from **Step 2**
...
1. Copy the **localhost URL** [http://localhost:4200](http://localhost:4200) from the **Command Prompt** and paste it in browser and press enter.
...
1. Once you get Logged-In you will be redirected to **ContosoAir** Website **.** Enter **Departure date** and **Return date** in **YYYY-MM-DD** format and click **Find Flights** button.
   ...
   > **Note:** If the browser refresh continously then please open the **localhost URL** [http://localhost:4200](http://localhost:4200) in different browser.
